...
<body>
		...
		<table>
			...
			<tr>
				...
				<td colspan="4" style="text-align: right">
					...
					<a href="javascript:void(0)" class="easyui-linkbutton" onclick="custListSearch()" iconCls="icon-search">查询</a></td>
			</tr>
		</table>
	</div>
	<table id="dg_cust_list" class="easyui-datagrid"		
	...
	<script type="text/javascript">
		$(function() {
			var cust_list_page = $('#dg_cust_list').datagrid().datagrid('getPager');
		...
		function doSearch(pageNumber,pageSize) {
			...
			$.ajax({
                ...
                success : function(data) {
                    //alert(data);
                	$("#dg_cust_list").datagrid("loadData",data);
                },
		...
		function openAddWindow() {
			parent.window.openWindow(0, '${ctx}/crm/customer_add');
		...
		function openModifyWindow(custId) {
			parent.window.openWindow(custId, '${ctx}/crm/customer_add?custId='+custId);
		...
		function openViewWindow(custId) {
			...
			$('#open_cust_detail').window({
				...
				closed : false,
				cache : false,
				href : '${ctx}/crm/customer_add?custId='+custId+'&operator=view',
				modal : true
			}); */
		}
